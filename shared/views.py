...
class SearchListView(ListView):
    model_fields_filter_class = None
    ...
    def get_queryset(self):
        ...
        if self.model_fields_filter_class is not None:
            queryset = self.model_fields_filter_class(self.request.GET, queryset=queryset).qs
        return queryset
...
class FilterListView(ListView):
    ...
    def get_model_fields_filter_class(self):
        if self.model_fields_filter_class is None:
            raise ImproperlyConfigured("model_fields_filter_class attribute is missing")
        return self.model_fields_filter_class
